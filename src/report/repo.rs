...
impl RepoOverview {
    ...
    pub async fn generate_report(&mut self) -> Result<(), ReportError> {
        ...
        // println!("Strings combined: {}", dates_str);
        // println!("Numbers combined: {}", numbers_str);
    ...
    let re1 = Regex::new(r#"var\s+data2\s+=\s+(.*?);"#).unwrap();
        ...
        let replaced_contents: Vec<String> = replaced_contents6.iter()
        .map(|line| {
            let line: Cow<str> = line.into();
